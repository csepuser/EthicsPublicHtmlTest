<?php
	...
	//$link = &AdoNewConnection( $system_db_dbtype );
	//$link->Connect( $system_db_server, $system_db_username, $system_db_password, $system_db_dbname );
...
function popup(mylink)
{
	...
	if (typeof(mylink) == 'string')
   		...
   		href=mylink.href;
	window.open(href, 'Help', 'width=600,height=600,scrollbars=yes');
	return false;
}
...
<div id="neb_search_form"> 
  ...
  <!--Rahul Trehan:Included the new feature AutoSuggest(11/21/2008)-->
  <script type="text/javascript">
			var options = {
	...
	if ($TotalCount != 0)
	{
		...
		else
		{
		  if($row == (integer)($TotalCount/10))
			...
			echo  '<td align=right >';
			echo "<strong>&nbsp; " . $Firstrow . ' - &nbsp; '.$Lastrow .' of &nbsp;'.$TotalCount ."&nbsp;</strong>";
			echo '</td>';
		}
		echo '</tr>';
		echo '</table>';				
...
<div id="content"> <br/>
    <table width="100%" cols="2" border="0">
      ...
      <td> 
        <?php
			...
			{	  
				  ...
				  echo '</strong></td></tr>';
				  echo '<tr> <td>';
				  //echo one_wordwrap($row->author,40);
				  ...
				  if (strcmp(trim($row->title_periodical),"") && isset($row->title_periodical))
					{
						echo '<tr> <td>';
						//echo stripcslashes(wordwrap($row->title_periodical,30,"\n",true));
				  ...
				  if (strcmp(trim($row->title_publisher),"") && isset($row->title_publisher))
					{
						echo '<tr> <td>';
						//echo wordwrap($row->publisher,30,"\n",true);
				  ...
				  if (strcmp(trim($dt_published->date_published),"") && isset($dt_published->date_published))
					{
						echo '<tr> <td>';
						//echo wordwrap(($dt_published->date_published),30,"\n",true);
				  ...
				  if (strcmp(trim($dt_published->url_source),"") && isset($dt_published->url_source))
					{
						echo '<tr> <td>';
						//echo stripcslashes(htmlspecialchars(wordwrap($dt_published->url_source,30,"-\n",true))); 
      ...
      <td valign="top"> <table border="1" bgcolor="#DBDBDB" cellspacing="2" cellpadding="6" width=100% >
...
<script type="text/javascript">
try {
var pageTracker = _gat._getTracker("UA-8938192-3");
