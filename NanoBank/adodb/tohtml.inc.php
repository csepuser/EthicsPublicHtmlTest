...
function rs2html(&$rs,$ztabhtml=false,$zheaderarray=false,$htmlspecialchars=true,$selected_column=1,$selected_column_html='****') {
    ...
    for ($i=0; $i < $ncols; $i++) {
        ...
        {
            if ( eregi( "input type=\"checkbox\"", ($field->name) ) )
            {
                $fname = ($field->name);            
            }
            else
        ...
        if ( eregi( "input type=\"checkbox\"", ($field->name) ) )
        {
            $hdr .= "<td class=tableheader>$fname";
        }
        else
        {
            $hdr .= "<td class=tableheader><a href='javascript: resort($i);'>$fname</a>";
        }
    ...
    print "<tr bgcolor=\"#FFFFFF\" class=\"class\">" . $hdr . "</tr>\n\n";
    ...
    while (!$rs->EOF) {
        ...
        $color_counter++;
        $classname = (( $color_counter % 2 ) == 1 ) ? "textplain" : "textBg";

        $s .= "<tr valign=top bgcolor=\"#FFFFFF\" class=\"class1\">\n";
        ...
        for ($i=0, $v=($numoffset) ? $rs->fields[0] : reset($rs->fields);
            ...
            $s .= "<td class=$classname align=\"left\">" . $v . "</td>\n";
    ...
    if (strlen($s) == 0) {
        $s = "<tr bgcolor=\"#FFFFFF\"><td colspan=13 class=textplain>No matches</td></tr>";
    }
