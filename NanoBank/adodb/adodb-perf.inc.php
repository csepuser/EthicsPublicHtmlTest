...
{
global $ADODB_LOGSQL,$HTTP_SERVER_VARS;
	...
	if (!empty($ADODB_LOGSQL)) {
		...
		$arr = array('b'=>trim(substr($sql,0,250)),
					'c'=>substr($sql,0,4000), 'd'=>$params,'e'=>$tracer,'f'=>round($time,6));
		
		$saved = $conn->debug;
		$conn->debug = 0;
		if ($conn->dataProvider == 'oci8' && $conn->databaseType != 'oci8po') {
			$isql = "insert into adodb_logsql values($conn->sysTimeStamp,:b,:c,:d,:e,:f)";
		} else if ($conn->databaseType == 'odbc_mssql') {
			$timer = $arr['f'];
...
Committed_AS:   348732 kB
	...
	*/	
	...
	{
		global $ADODB_LOGSQL,$ADODB_FETCH_MODE,$HTTP_GET_VARS;
		if (!empty($ADODB_LOGSQL)) {
			$saveE = $this->conn->fnExecute;
			$this->conn->fnExecute = false;
			...
			$s = "<h3>Suspicious SQL</h3>\n<table border=1 bgcolor=white><tr><td><b>Avg Time</b><td><b>SQL</b><td><b>Count</b><td><b>Max</b><td><b>Min</b></tr>\n";
			while (!$rs->EOF) {
				$sql = trim($rs->fields[1]);
				
				$prefix = "<a href=\"?exps=1&sql=".rawurlencode($sql)."&x#explain\">";
				$suffix = "</a>";
				if ($this->explain == false || strlen($prefix)>2000) {
					$prefix = '';
					$suffix = '';
				}
				$s .= "<tr><td>".round($rs->fields[0],6)."<td><font size=-1>".$prefix.htmlspecialchars($sql).$suffix."</font><td align=right>".$rs->fields[2].
	...
	*/
	...
	{
		global $ADODB_LOGSQL,$HTTP_GET_VARS,$ADODB_FETCH_MODE;
		...
		if (!empty($ADODB_LOGSQL)) {
			...
			$ADODB_FETCH_MODE = ADODB_FETCH_NUM;
			$rs =& $this->conn->SelectLimit(
			"select avg(timer)*count(*) as total,$sql1,count(*),max(timer) as max_timer,min(timer) as min_timer
				from adodb_logsql
				where {$this->conn->upperCase}({$this->conn->substr}(sql0,1,5))  not in ('DROP ','INSER','COMMI','CREAT')
				and (tracer is null or tracer not like 'ERROR:%')
				group by sql1
			...
			$ADODB_FETCH_MODE = $save;
			if (!$rs) return "<p>$this->helpurl. ".$this->conn->ErrorMsg()."</p>";
			$s = "<h3>Expensive SQL</h3>\n<table border=1 bgcolor=white><tr><td><b>Load</b><td><b>SQL</b><td><b>Count</b><td><b>Max</b><td><b>Min</b></tr>\n";
			while (!$rs->EOF) {
				$sql = $rs->fields[1];
				
				$prefix = "<a href=\"?expe=1&sql=".rawurlencode($sql)."&x#explain\">";
				$suffix = "</a>";
				if($this->explain == false || strlen($prefix>2000)) {
					$prefix = '';
					$suffix = '';
