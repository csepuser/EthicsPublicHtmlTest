  ...
  /*  function RenderLayout($header,$grid,$footer,$attributes='class=widget cellspacing=1 cellpadding=0 border=0 width="100%"')
    {
        ...
        if (($this->how_many_rows > 0) &&
           (($this->id == 'opportunities')
            or ($this->id == 'activities')
            or ($this->id == 'contacts') ))
        {
           // echo "<tr><td class=widget_content_form_element colspan=10><input type=button class=button onclick=\"javascript: exportIt();\" value='Export'> <!--<input type=button class=button onclick=\"javascript: bulkEmail();\" value='Mail Merge'>--></td></tr>";
        }

        echo "</table>";
    }*/
    
    function RenderLayout( $header, $grid, $footer, $attributes='class=widget cellspacing=0 border=0 width="100%"' )
    {
        $pager_rows = "<table " . $attributes . ">";
        $pager_rows . = "<tr><td colspan=13 class=widget_header> Search Results </td></tr>\n";
        
        if( $header != '&nbsp;' )
        {
            $pager_rows .= "<tr><td colspan=13>";
            $pager_rows .= "<table border=0 cellspacing=0 cellpadding=0 width=100%>";
            $pager_rows .= "<tr><td class=widget_label>" . $footer . "</td>" . "<td clign=right class=widget_label>" . $header . "</td></tr>";
            $pager_rows .= "</table>";
            $pager_rows .= "</td></tr>\n";
        }
